...
const Profile: React.FC = () => {
  ...
  const savedLists = currentUser?.save
    ? currentUser.save.map((savedItem: any) => ({
        ...savedItem.list,
        creator: savedItem.list?.creator || { Name: "Unknown" }, // Handle missing creator
      }))
    : [];
              ...
              {activeTab === "liked" && (
                ...
                >
                  ...
                  {userLikedLists.length > 0 ? (
                    <GridListList lists={userLikedLists?.map(item => ({
                      ...item,
                      creator: item?.creator || { Name: 'Unknown Creator' } // Safe access
                    }))} showUser={true} />
                  ) : (
